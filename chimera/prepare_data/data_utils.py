...
def gen_config_yaml(
    ...
    cur_dir = op.abspath(cur_dir)
    writer = S2TDataConfigWriter(op.join(cur_dir, yaml_filename))
    writer.set_audio_root(data_root)
    ...
    writer.set_bpe_tokenizer(
        {
            "bpe": "sentencepiece",
            "sentencepiece_model": op.join(cur_dir, spm_filename),
        }
    )
    if src_spm_filename is not None:
        ...
        writer.set_src_bpe_tokenizer(
            {
                "bpe": "sentencepiece",
                "sentencepiece_model": op.join(cur_dir, src_spm_filename),
            }
        )
