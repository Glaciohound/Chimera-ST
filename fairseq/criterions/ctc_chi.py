...
class CtcCriterion(LegacyFairseqCriterion):
    def __init__(self, args, task):
        ...
        if args.wer_args is not None:
            from chi.examples.speech2text.wave2vec_libri.w2l_decoder import \
                W2lKenLMDecoder, W2lFairseqLMDecoder
            ...
            if 'transformer' in args.wer_args:
                self.w2l_decoder = W2lFairseqLMDecoder(dec_args, task.target_dictionary)
            elif 'kenlm' in args.wer_args:
                self.w2l_decoder = W2lKenLMDecoder(dec_args, task.target_dictionary)
            else:
                raise Exception("model not supported" + args.wer_args)
        else:
