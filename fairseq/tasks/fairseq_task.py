...
import torch
from fairseq import metrics, search, tokenizer, utils, scoring
...
class FairseqTask(object):
    ...
    def filter_indices_by_size(
        ...
        """
        logger.info("filtering indices by size")
    ...
    def get_batch_iterator(
        ...
        if keep_num_indices is not None and keep_num_indices <= len(indices):
            if self.meta_indices is None:
                self.meta_indices = np.random.choice(
                    np.arange(indices.shape[0]),
                    keep_num_indices,
                    replace=False,
                )
                self.meta_indices.sort()
            indices = indices[self.meta_indices]
            logger.info(f"randomly selecting {keep_num_indices}")

        # create mini-batches with given size constraints
        logger.info("creating mini-batches given size constraints")
        ...
        logger.info(f"counted {len(batch_sampler)} batches")

        # return a reusable, sharded iterator
        logger.info("creating EpochBatchIterator")
    ...
    def build_generator(
        self, models, args, seq_gen_cls=None,
        ...
        from fairseq.controlled_sequence_generator import \
            ControlledSequenceGenerator
        ...
        if seq_gen_cls is None:
            ...
            elif args.controlled_generator:
                seq_gen_cls = ControlledSequenceGenerator
                extra_gen_cls_kwargs['bpe'] = self.build_bpe(None)
            else:
    ...
    def build_scorer(self, scorer_config, tgt_dict):
        return scoring.build_scorer(scorer_config, tgt_dict)
...
class LegacyFairseqTask(FairseqTask):
    ...
    def build_tokenizer(self, tokenizer_config):
        return encoders.build_tokenizer(tokenizer_config)

    def build_bpe(self, bpe_config):
        return encoders.build_bpe(bpe_config)
    ...
    def shift_other_variables(self, *args):
        return args

    def resetup(self, cfg):
        pass
