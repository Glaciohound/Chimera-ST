...
from fairseq.models.wav2vec import (
    Wav2Vec2Model, Wav2VecCtc,
    Wav2Vec2QuasiwaveModel
)
...
class S2T_W2V2_TransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args):
        ...
        if not self.use_asr_finetune_w2v: # if use ssl-trained only
            self.w2v_args = ckpt["args"]
            if getattr(args, 'wav2vec2_quasiwave', False):
                self.wav2vec_model = Wav2Vec2QuasiwaveModel.build_model(
                    ckpt['args'], task=None)
            else:
